...
function getLottoNumbers() {    // 로또 추출 함수
  let lottoNumbers = [];        // 빈 배열 선언
  while (lottoNumbers.length < LOTTO_COUNT) {   // 7개를 추출할 때까지 반복
    let random = Math.floor(Math.random() * MAX_NUM) + 1;   // 1~45 중 임의의 수를 선택
    if (!lottoNumbers.includes(random)) {   // 뽑았던 수가 아니라면
...
date.innerHTML += `<span>${today.getFullYear()}-${today.getMonth()+1}-${today.getDate()} 추첨</span>`;               // 연, 월, 일 추가

// 버튼 클릭 이벤트에 연결
const btn = document.getElementById("btn");     // 버튼 선택
const result = document.getElementById("result"); // result 부분 선택
btn.addEventListener("click", function () {     // 클릭 이벤트 발생 시 실행할 함수
  result.innerHTML = "";                        // 초기화 후 시작
  let lottoNumbers = getLottoNumbers();         // 로또 번호 선택
  for (let i = 0; i < lottoNumbers.length; i++) { // 7번 반복
    setTimeout(() => {  // 공을 하나씩 보여주기 위한 시간 설정 함수
